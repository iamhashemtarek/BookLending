...
{
    public class GetUserBorrowHistoryByIdHandler : IRequestHandler<GetUserBorrowHistoryByIdQuery, IEnumerable<BorrowDto>>
    {
        ...
        public GetUserBorrowHistoryByIdHandler(IUnitOfWork unitOfWork, IMapper mapper, ILogger<IBorrowService> logger, IOptions<BorrowSettings> options)
        {
        ...
        public async Task<IEnumerable<BorrowDto>> Handle(GetUserBorrowHistoryByIdQuery request, CancellationToken cancellationToken)
        {
            var spec = new UserBorrowSpecification(request.UserId);
